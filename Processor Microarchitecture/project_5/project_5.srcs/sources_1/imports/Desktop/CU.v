module CU(
	input cnt_out, R_lt_Y, GO, ZeroDivisor, clk, rst,
	output reg ERROR, DONE, CTE,
	output reg LD, RST, xRightIn, SL, LDR, SRR, r2MuxSel, r1MuxSel,
	output reg [3:0] CT);
	
	reg [2:0] NS, CS;
	reg [14:0] ctrl;
	
	always @ (ctrl) {ERROR, DONE, LD, RST, xRightIn, SL, LDR, SRR, r2MuxSel, r1MuxSel, CT, CTE} = ctrl;
	...
	always @(CS, NS, GO, ZeroDivisor, R_lt_Y, cnt_out) begin
		case(CS)
			S0: begin
				if(!GO && !ZeroDivisor) begin 
					NS = S0; 
					ctrl = IDLE;
				end
				else if (ZeroDivisor) begin 
					NS = S0; 
					ctrl = error; 
				end
				else begin 
